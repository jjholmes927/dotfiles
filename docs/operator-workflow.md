...
| Window | Runs | Job |
|--------|------|-----|
| `fleet` | `claude agents --cwd $ENG_ROOT --dangerously-skip-permissions` | The inbox. Never closes — needs-input/completed notifications only fire while it is open. The flag applies to sessions dispatched from the view (`@lane <prompt>`), which otherwise land in auto mode and block on the first permission prompt — `new-agent`'s flag does not reach them. |
...
| Cheat sheet | `tmux/deck-cheatsheet.txt` + `tmux/deck-cheat` | The plain-text source and its ANSI colouriser (headers blue, commands green, destructive bits red, caveats amber). Both the ops pane and the popup run `deck-cheat`, never `cat` — keep the `.txt` plain so it stays diffable and width-checkable. The command + fleet-key reference shown in the ops pane, and in a `prefix ?` popup from **any** window — the fleet window has no ops pane, so the popup is the only way to read the fleet keys without leaving the inbox. Hard-wrapped to 42 cols to fit the 44-col ops pane; keep it that way. The ops pane `cat`s the file once at pane creation, so an already-running deck shows the old text until the pane is recreated. |
