...
| `deck` · cheat pane (right top, 46 cols) | `deck-cheat`, rendered once at deck-time | The command + fleet-key reference, clipped from the bottom to whatever the pane can hold. `prefix ?` pops up the full sheet from any window. |
| `deck` · ops pane (right bottom, 46 × 14) | bash, auto-runs `clone-status` | `new-agent`, `clone-status`, `gmp-all`, `claude rm`, git surgery. 14 rows is enough for a prompt and a short table; `prefix z` zooms it full-window for anything wide. |
