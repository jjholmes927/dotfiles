...
4. **Findings and data are told as a story before any number** (chat, Linear, docs alike): start with the user-facing problem, then what the feature does, then what each term means (define every domain word the first time: destroy, rescue, shadow…), then derive the headline number and show its arithmetic, then why it can or cannot be trusted, then the one-line honest summary. Never open with a bare metric like "74% accept rate". End a recap with "does that land, or go slower on a step?". Evidence tables come after the story.
